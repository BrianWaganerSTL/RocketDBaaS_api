...
def IncidentCleanup():
  ...
  for i in issueTracker:
    ...
    i.current_status = Incident.StatusChoices.UNKNOWN
    i.pending_status = Incident.StatusChoices.UNKNOWN
    i.save()
    ...
    myHtmlTemplate = '''<h1><span style="%s">%s</span> Server: %s</h1>\n
                    ...
                    IssueTrackerId: %s <br>\n
                    <br>\n
                    Link to Issue: <a href="http://0.0.0.0:8000/clusters/%s/servers/%s/incidents/%s">Click</a> <br>\n
                    '''
