...
from dbaas.models import IssueNotification, Server, IssueTracker, IssueStatusChoices, ApplicationContact, CheckerThreshold, MetricsLoad
...
def Track_Load(slimServer, metrics_load_id):
    metricsLoad = MetricsLoad.objects.get(id=metrics_load_id)

    try:
        chkrThreshold = CheckerThreshold.objects\
            .filter(active_sw=True,
                    checker_base_element__active_sw=True,
                    checker_base_element__metric_name='Load',
                    checker_base_element__metric_element='load_5min',
                    server_override__isnull=True)[0]
    except:
        logging.error('Found no active Checker for Load (load_5min)')
        return
    else:
        load_5min = metricsLoad.load_5min
        if DynCompare(load_5min, chkrThreshold.critical_predicate_type, chkrThreshold.critical_value):
            thresholdLevel = IssueStatusChoices.Critical
            thresholdTestWithValues = '%d %s %s' % (load_5min, chkrThreshold.critical_predicate_type, chkrThreshold.critical_value)
            print('%s Threshold' % thresholdLevel)
        elif DynCompare(load_5min, chkrThreshold.warning_predicate_type, chkrThreshold.warning_value):
            thresholdLevel = IssueStatusChoices.Warning
            thresholdTestWithValues = '%d %s %s' % (load_5min, chkrThreshold.warning_predicate_type, chkrThreshold.warning_value)
            print('%s Threshold' % thresholdLevel)
        else:
            thresholdLevel = IssueStatusChoices.Normal
            thresholdTestWithValues = '%d %s %s' % (load_5min, chkrThreshold.normal_predicate_type, chkrThreshold.normal_value)
