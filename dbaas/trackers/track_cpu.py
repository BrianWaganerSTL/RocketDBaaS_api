...
def Track_Cpu(slimServer, metrics_cpu_id):
    metricsCpu = MetricsCpu.objects.get(id=metrics_cpu_id)

    try:
        chkrThreshold = CheckerThreshold.objects\
            .filter(active_sw=True,
                    checker_base_element__active_sw=True,
                    checker_base_element__metric_name='Cpu',
                    checker_base_element__metric_element='cpu_idle_pct',
                    server_override__isnull=True)[0]
    except:
        logging.error('Found no active Checker for CPU (cpu_idle_pct)')
        return
    else:
        cpu_idle_pct = metricsCpu.cpu_idle_pct
        if DynCompare(cpu_idle_pct, chkrThreshold.critical_predicate_type, chkrThreshold.critical_value):
            thresholdLevel = IssueStatusChoices.Critical
            thresholdTestWithValues = '%d %s %s' % (cpu_idle_pct, chkrThreshold.critical_predicate_type, chkrThreshold.critical_value)
            print('%s Threshold' % thresholdLevel)
        elif DynCompare(cpu_idle_pct, chkrThreshold.warning_predicate_type, chkrThreshold.warning_value):
            thresholdLevel = IssueStatusChoices.Warning
            thresholdTestWithValues = '%d %s %s' % (cpu_idle_pct, chkrThreshold.warning_predicate_type, chkrThreshold.warning_value)
            print('%s Threshold' % thresholdLevel)
        else:
            thresholdLevel = IssueStatusChoices.Normal
            thresholdTestWithValues = '%d %s %s' % (cpu_idle_pct, chkrThreshold.normal_predicate_type, chkrThreshold.normal_value)
